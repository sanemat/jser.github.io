...
`<template>`要素の`showroot`属性で宣言的にShadow Root宣言できるようになる[Proposal](https://github.com/mfreed7/declarative-shadow-dom)です。
...
`<template>`要素の`showroot`属性で宣言的にShadow Root宣言できる。
